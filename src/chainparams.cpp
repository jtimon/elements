...
static std::vector<unsigned char> CommitToArguments(const std::string& networkID, const CScript& fedpegscript, const CScript& signblockscript)
{
    CRIPEMD160 ripemd;
    unsigned char commitment[20];
    ripemd.Write((const unsigned char*)networkID.c_str(), networkID.length());
    ripemd.Write((const unsigned char*)HexStr(fedpegscript).c_str(), HexStr(fedpegscript).length());
...
static CBlock CreateGenesisBlock(const CScript& scriptCommit, const CScript& genesisOutputScript, uint32_t nTime, const CScript& scriptChallenge, int32_t nVersion, const CAmount& genesisReward, const uint32_t rewardShards)
{
    ...
    txNew.vin[0].scriptSig = scriptCommit;
...
public:
    ...
    void Reset(const std::map<std::string, std::string>& mapArgs)
    {
        ...
        std::vector<unsigned char> commitment = CommitToArguments(strNetworkID, consensus.fedpegScript, genesisChallengeScript);
        genesis = CreateGenesisBlock(CScript(commitment), scriptDestination, 1231006505, genesisChallengeScript, 1, MAX_MONEY, 100);
...
public:
    ...
    void Reset(const std::map<std::string, std::string>& mapArgs)
    {
        ...
        std::vector<unsigned char> commitment = CommitToArguments(strNetworkID, consensus.fedpegScript, genesisChallengeScript);
        genesis = CreateGenesisBlock(CScript(commitment), defaultRegtestScript, 1296688602, genesisChallengeScript, 1, MAX_MONEY, 100);
        consensus.hashGenesisBlock = genesis.GetHash();
